/**
 * Proposal Regeneration API Route
 * POST /api/projects/[projectId]/proposals/[proposalId]/regenerate
 */
...
export async function POST(request, { params }) {
  ...
  const rl = rateLimit({
    key: `ai_regenerate:${session.user.id}`,
    limit: 10,
    windowMs: 60_000,
  });
  if (!rl.ok) {
  ...
  try {
    ...
    if (gitProvider === "github" && !token) {
      return NextResponse.json(
        { error: "GitHub not connected. Connect GitHub before regenerating proposals." },
        { status: 401 }
      );
    }
    ...
    const updatedProposal = await prisma.proposal.update({
      ...
      data: {
        ...
        summary,
        modelLabel:
          scanResult.modelLabel ||
          (process.env.OPENAI_API_KEY ? "OpenAI (app key)" : "Built-in generator"),
        generationMetadata: {
        ...
        files: {
          deleteMany: {},
          ...(proposalFiles.length ? { create: proposalFiles } : {}),
        },
      },
      include: { files: true },
    });
    ...
    return NextResponse.json(formatProposal(updatedProposal));
  } catch (error) {
    console.error("Failed to regenerate proposal:", error);
    return NextResponse.json(
      { error: "Failed to regenerate proposal", message: error.message },
      { status: 500 }
    );
  }
}
